...
export async function POST(req) {
  ...
  try {
    ...
    // Construct parameterized query to prevent SQL Injection
    const query = `
  INSERT INTO Testimonials (Name, Email, Testimonial, Rating, Image, Active, Company)
  OUTPUT INSERTED.*
  VALUES (@Name, @Email, @Testimonial, @Rating, @Image, '0', @Company);
`;

    const request = pool.request();
    request.input("Name", formData?.Name);
    request.input("Email", formData?.Email);
    request.input("Testimonial", formData?.Testimonial);
    request.input("Rating", formData?.Rating);
    request.input("Image", formData?.Image || null);
    request.input("Company", formData?.Company || null);

    console.log("Executing Query:", query);

    const result = await request.query(query);
    insertedRow = result.recordset[0]; // Get the first (and only) row inserted

    console.log("Inserted Row:", insertedRow);

    // Sending WhatsApp message after successful insertion
    ...
    const messageResponse = await SendWhatsAppMessgae(
      "AVYN",
      "9799318629",
      "testimonial_msg2",
      [
        { type: "text", text: `${formData.Name} (${formData.Company})` }, // Name with Company in brackets
        { type: "text", text: formData.Email },
        { type: "text", text: formData.Testimonial },
        { type: "text", text: formData.Rating },
        {
          type: "text",
          text: `http://erp.autovyn.com/backend/Website/TestimonialMsg?compcode=AVYN&UTD=${insertedRow.UTD}`,
        },
      ]
    );
  }
